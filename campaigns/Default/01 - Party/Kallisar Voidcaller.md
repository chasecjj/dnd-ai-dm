---
type: party_member
name: Kallisar Voidcaller
player: derienkraus
pronouns: he/him
race: Half-Elf
class: Warlock
subclass: Pact of the Blade
level: 1
hp_current: 10
hp_max: 10
ac: 14
conditions: []
spell_slots_used: 0
spell_slots_max: 1
pact_magic: true
tags:
- party
---
# Kallisar Voidcaller

## Stats
| Stat | Score | Mod |
|------|-------|-----|
| STR  | 12    | +1  |
| DEX  | 14    | +2  |
| CON  | 12    | +1  |
| INT  | 13    | +1  |
| WIS  | 10    | +0  |
| CHA  | 16    | +3  |

## Abilities & Features
- **Pact of the Blade:** Bonus Action to conjure a pact weapon or bond with a magic weapon. Can deal Necrotic, Psychic, or Radiant damage instead of normal type.
- **Pact Magic:** 1 spell slot (1st level), recharges on short rest.
- **Spell Save DC:** 11
- **Spell Attack Bonus:** +3

## Prepared Spells
- **Eldritch Blast** (Cantrip) — Ranged spell attack, 1d10 force damage.
- **Hex** (1st level) — Bonus Action, extra 1d6 necrotic on hits + disadvantage on one ability.

## Inventory
- Leather Armor (AC 11 + DEX)
- Dagger (steel)
- Pact Weapon (currently: Shortsword — can shift forms)
- **Flask of Oil**
- "Fun Toys" sack — _Contains miscellaneous useful items_
- Bolt case (20 bolts)

## Personality
- **Scholarly and tactical** — first instinct is to scan the room for advantages.
- Loyal to Frognar, acts as the "enabler/investigator" of the pair.
- Player explicitly requested using **Investigation** in combat — wants environmental interaction.
- Stylish and acrobatic in combat (leaping attacks, weapon transformations mid-fight).

## Bonds & Hooks
- Investigator personality — drawn to mysteries and puzzles.
- Interest in "cursed trinkets" shared with Frognar.
- Close friendship with [[Frognar Emberheart]].
